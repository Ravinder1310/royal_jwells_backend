...
export const createProductController = async (req, res) => {
  try {
    ...
    const { name, slug, description, price, offer,offerPrice, category } =
      req.body;
...
export const productPhotoController = async (req, res) => {
  try {
    const product = await ProductModel.findById(req.params.pid).select("-photo.data");
    if (product.photo.data) {
      
      return res.status(200).send({ photo: product.photo });;
    }
  } catch (error) {
    console.log(error);
    res.status(500).send({
...
export const deleteProductController = async (req, res) => {
  try {
    await ProductModel.findByIdAndDelete(req.params.pid).select("-photo.data");
    res.status(200).send({
      success: true,
...
export const similarProductController = async(req,res) => {
  try {
    ...
    const products = await ProductModel.find({
      category:cid,
      _id:{$ne:pid}
    }).select('-photo.data').limit(3).populate('category');
    res.status(200).send({
      success:true,
      products
    })
